...
>((props, ref) => {
  const {
    ...
    className = "",
    style,
    selectionBoxClassName = "border border-blue-500 bg-blue-100/10",
    selectedItemClassName = "bg-blue-100/30 transition-colors duration-200",
    contextMenuContent,
  } = props;
  ...
  return (
    <>
      <div
        ref={containerRef}
        className={`relative w-full h-full ${className} select-none`}
        style={style}
      >
        {children}
        {isSelecting && isDragging.current && (
          <div
            className={`absolute pointer-events-none z-10 ${selectionBoxClassName} selection-box marquee-selection-internal`}
            style={{
      ...
      {menuProps.visible && (
        <div
          className="fixed z-50 marquee-selection-menu"
          style={{
            left: menuProps.x,
            top: menuProps.y,
            position: "fixed",
